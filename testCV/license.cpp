...
typedef struct tLicensePlateCharacterFeatures_tag {
	bool number;					//判断是否是个数字
	CvRect boundaryRect;
	int xCentre;					//中心坐标
	int yCentre;
	int n_Holes;					//孔的数量
	int areaRect;					//矩形区域
	float chRatio;					//凸包/矩形比
	int n_conc;						//concavitues的数量
	int conc_ypos[5];				//Concavities(凹陷)数据
	int conc_dir[5];
	int conc_ystart[5];
	int conc_ydepth[5];
	int conc_depth[5];
	int hole_ypos[5];				//孔 y 位置
...
//获取样本特征数据, 此函数仅在开发阶段使用,在项目发布时不会使用.
...
int main3( int argc, char** argv )
{
	...
	{
		char filename[100];
		sprintf(filename,"./RealNumbers/%d.jpg",character); //./Real Numbers/%d.jpg
